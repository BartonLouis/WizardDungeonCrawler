    ...
    public class BlackboardDataEditor : Editor {
        ...
        private void OnEnable() {
            entryList = new ReorderableList(
                serializedObject, serializedObject.FindProperty("entries"),
                true, true, true, true) {
                drawHeaderCallback = rect => {
                    EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width * 0.3f, EditorGUIUtility.singleLineHeight), "Key");
                    EditorGUI.LabelField(new Rect(rect.x + rect.width * .3f + 10, rect.y, rect.width * 0.3f, EditorGUIUtility.singleLineHeight), "Type");
                    EditorGUI.LabelField(new Rect(rect.x + rect.width * .6f + 5, rect.y, rect.width * 0.4f, EditorGUIUtility.singleLineHeight), "Value");
                }
            };
            ...
            entryList.drawElementCallback = (rect, index, isActive, isFocused) => {
                ...
                var keyNameRect = new Rect(rect.x, rect.y, rect.width * .28f, EditorGUIUtility.singleLineHeight);
                var valueTypeRect = new Rect(rect.x + rect.width * .3f, rect.y, rect.width * .28f, EditorGUIUtility.singleLineHeight);
                var valueRect = new Rect(rect.x + rect.width * .6f, rect.y, rect.width * .38f, EditorGUIUtility.singleLineHeight);
